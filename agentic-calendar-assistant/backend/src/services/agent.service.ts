...
function getModelConfig(): string {
  const provider = (
  ...
  if (provider === "openrouter") {
    ...
    const rawModel = process.env.AI_MODEL ?? "google/gemini-2.0-flash-exp:free";
    const model = rawModel.startsWith("openrouter/")
      ? rawModel
      : `openrouter/${rawModel}`;
    return model;
  }
  ...
  if (provider === "google") {
    ...
    if (!key) {
      throw new Error(
        "Google Gemini API key is missing. Please set GOOGLE_GENERATIVE_AI_API_KEY, GOOGLE_API_KEY, or GOOGLE_GEMINI_API_KEY in .env",
      );
    }

    // Set standard env vars for Mastra's Google SDK
    ...
    const model = rawModel.startsWith("google/")
      ? rawModel
      : `google/${rawModel}`;
  ...
  return rawModel.startsWith("openai/") ? rawModel : `openai/${rawModel}`;
}
    ...
    if (chunk.type === "error") {
      ...
      const errMsg =
        typeof chunk.payload === "string"
          ? chunk.payload
          : (chunk.payload as { error?: { message?: string }; message?: string })?.error?.message ||
            (chunk.payload as { error?: { message?: string }; message?: string })?.message ||
